...
I used the linear regression and XGBoost models to generate measures of variable importance. (PCR and the ensemble do not provide variable importance metrics.) I then examined the top five most important variables and compared them to see if the two models found the same variables to be important.

# 2. Methods and Analysis

## 2.1 Exploratory Analysis and Variable Selection

I examined the structure of the data. The dataset contains 3203 rows and 69 variables: 67 predictor variables, one outcome variable and the respondent number. All variables are shown as numeric, although some should be factor type. I will convert them later on.
...
*ANOVA*. For variables that were nominal or that were ordinal with fewer than 5 responses, I ran ANOVAs to determine whether there was a statistically significant difference (p\<.05) in mean loneliness across levels. The 7 relevant variables were:
...
All seven variables showed significant differences in mean loneliness among their levels (p\<.01 or better.) As a result, I included all of them in my models. Below is the code for running the seven ANOVAs.
...
I examined the top five most important variables in each model to see where there were commonalities.
...
The linear regression model yields an adjusted R-squared of 41.5% and an RMSE in the test set of 7.950. The table below compares these results with the AARP linear regression. The AARP model, which contained over 50 variables yielded an R-squared of 21.3%. My linear regression model explained almost twice as much of the variance in the outcome variable.
